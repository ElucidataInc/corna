...
# read maven data
maven_files = corna.read_maven(path_dir + '/maven_output.csv')

# read maven metadata file
maven_metadata = corna.read_mvn_metadata(path_dir + '/metadata.csv')

# merge maven files and metadata files
merge_mv_metdata = corna.merge_mv_metadata(maven_files, maven_metadata)
...
filtered_data = corna.filtering_df(merge_mq_metdata, num_col=2, col1="Name",
